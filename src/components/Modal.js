...
export const Modal = ({setModal, food}) => {
  ...
  const {id, name, img, price, description, rating, quantity} = food;
  const dispatch = useDispatch()

  const addToCart = (id, name, image, price, quantity) => {
    dispatch(cartActions.addToCart({id, name, image, price, quantity}))
  ...
  return (
    <div className='fixed top-0 left-0 w-full h-full bg-black/70 z-50 '>
        ...
        <div>
              ...
              <img src={img} alt="product" className='w-[600px] h-[300px] object-cover rounded'/>
              <div className='p-3'>
                ...
                <div className='flex justify-end text-orange-500 my-4'>
                  ...
                  <IoMdAddCircle size={25} className='mr-3 ml-2 cursor-pointer hover:text-orange-700' onClick={() => addToCart(id, name, img, price, quantity)}/>
                </div>
              </div>
        </div>        
      </div>
